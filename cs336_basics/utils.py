...
def save_with_pickle(
    ...
    try:
        ...
        with open(vocab_filepath, 'wb') as f: # 'wb' for binary write
            pickle.dump(vocab, f)
        if logger:
            logger.info(f"Vocabulary saved to {vocab_filepath}")
        else:
        ...
        if logger:
            logger.info(f"Merges saved to {merges_filepath}")
        else:
...
def load_with_pickle(
    ...
    try:
        ...
        if logger:
            logger.info(f"Vocabulary loaded from {vocab_filepath}")
        else:
        ...
        if logger:
            logger.info(f"Merges loaded from {merges_filepath}")
        else:
...
def get_pair_stats(
    ...
    Returns
    -------
    Tuple[Counter[Tuple[bytes, bytes]], int]
        A tuple containing a Counter object representing the frequency of pairs of pretoken sequences
        and an integer representing the total number of tokens processed.
        For instance, if the input is {b"abc": ([b"a", b"b", b"c"], 3)},
        the output will be {(b"a", b"b"): 3, (b"b", b"c"): 3}.
    """
    frequency_counter = Counter()
    if logger:
        logger.info("Getting by pair frequency count for {} pretoken keys".format(len(pretoken_freq)))
    else:
        print("Getting by pair frequency count for {} pretoken keys".format(len(pretoken_freq)))
    num_byte_tokens_total = 0
    # Iterate through each pretoken and its frequency
    for _, (tokens, freq) in pretoken_freq.items():
        num_byte_tokens_total += len(tokens)
    ...
    if logger:
        logger.info("Processed {} pretokens".format(num_byte_tokens_total))
    else:
        print("Processed {} pretokens".format(num_byte_tokens_total))
    return frequency_counter, num_byte_tokens_total
...
def merge_byte_pairs(
    ...
    num_byte_tokens_total = 0
    for pretoken, (byte_list_tokens, freq) in pretoken_freq.items():
        num_bytes = len(byte_list_tokens)
        num_byte_tokens_total += num_bytes
    ...
    # Return the current number of tokens processed
    # For logging how the tokens are merged
    # This is useful for tracking progress in the BPE process.
